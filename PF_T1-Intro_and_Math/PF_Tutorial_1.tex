...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Stop me if you're confused
...
\begin{frame}
  ...
  Here's how. Suppose we have a multi-variable function \(f : \mathbb{R}^n \supseteq U \to \mathbb{R}\), it's good enough that we could find
  an extrema under constraints \(\phi_j : U \to \mathbb{R}^m\) where \(m<n\).
 
  First, we construct a helper function
  \[F : \mathbb{R}^{n+m}\supseteq U' \to \mathbb{R},\]
  \[(x,\lambda) \mapsto f(x) - \sum_{i=1}^m \lambda_i \cdot \phi_i(x). \]
  Here, \(x\in U\) and \(\lambda \in \mathbb{R}^m\), the latter one is the `Lagrange Multiplier(s)'.
  We can see \(F\) contains the original function and the constraints.
\end{frame}
...
\begin{frame}
  \frametitle{For those who interested}
...
\begin{frame}
  ...
  \begin{block}{Q1}
    Suppose there are three functions:\(\phi,\psi : \mathbb{R}^3 \to \mathbb{R}\) and \(\hat{f} : \mathbb{R}^3 \to \mathbb{R}^3\).
    Please prove:
    \[\nabla\left( \phi\psi \right) = \phi\nabla\psi + \psi\nabla\phi\]
    \[\nabla\cdot\left( \phi\hat{f} \right) = \nabla\phi\cdot\hat{f} + \phi\nabla\cdot\hat{f}\]
    By proving these, you should understand what we meant by `derivative w.r.t. coordinates'.
    \bigbreak
